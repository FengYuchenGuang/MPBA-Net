...
class BoundaryCrossAttention(CrossAttention):
    def __init__(self,
        ...
        if self.BAG_type == '1D':
            if Atrous:
                self.BAG = BoundaryAwareAttentionGateAtrous1D(d_model)
            else:
                self.BAG = BoundaryAwareAttentionGate1D(d_model)
        elif self.BAG_type == '2D':
            if Atrous:
                self.BAG = BoundaryAwareAttentionGateAtrous2D(d_model)
            else:
                self.BAG = BoundaryAwareAttentionGate2D(d_model)
...
class BoundaryAwareAttentionGateAtrous1D(nn.Module):
    def __init__(self, in_channels, hidden_channels = None):

        super(BoundaryAwareAttentionGateAtrous1D,self).__init__()
...
class BoundaryAwareAttentionGateAtrous2D(nn.Module):
    def __init__(self, in_channels, hidden_channels=None):

        super(BoundaryAwareAttentionGateAtrous2D, self).__init__()
...
class BoundaryAwareAttentionGate1D(nn.Sequential):
    def __init__(self, in_channels, hidden_channels = None):
        super(BoundaryAwareAttentionGate1D,self).__init__(
    ...
    def forward(self, x):
        " x.shape: B, C, L "
        " return: feature, weight (B,C,L) "
        weight = torch.sigmoid(super(BoundaryAwareAttentionGate1D,self).forward(x))
        x = x * weight + x
        return x, weight



class BoundaryAwareAttentionGate2D(nn.Sequential):
    def __init__(self, in_channels, hidden_channels = None):
        super(BoundaryAwareAttentionGate2D,self).__init__(
    ...
    def forward(self, x):
        " x.shape: B, C, H, W "
        " return: feature, weight (B,C,H,W) "
        weight = torch.sigmoid(super(BoundaryAwareAttentionGate2D,self).forward(x))
        x = x * weight + x
        return x, weight
